...
BinarySearchTree<CountdownTree.Node<T>, T> implements SSet<T> {
	...
	public boolean add(T x) {
		...
		if (super.add(u)) {
			Node<T> w = u.parent;
		//	System.out.print(w.timer);
			while (w != nil){ //Iterates through until w is null
				w.timer--;
	...
	public void splice(Node<T> u) {
		...
		while (w != nil){ //Iterates through until w is null
			...
			if(w.timer == 0)
				this.explode(w); //If timer is 0, rebuilds

			w = w.parent; //Iterates to the next node
		}
		// add some code here (we just removed u from the tree)
	}

	protected void explode(Node<T> u) {
		this.rebuild(u); //Basically rebalancing
